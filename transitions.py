...
UseCase = Literal["UC1", "UC2", "UC3", "NUIS"]
...
# ---------------------------------------------------------------------------
# Flight profile -- an axis orthogonal to the use case
# ---------------------------------------------------------------------------
#
# WHY THIS EXISTS. The first production dataset contained no danger at all:
# `lbl_risk_class` was `nominal` on all 25,260,000 rows, and `gt_envelope_state`
# agreed. Every episode flew cruise at 77 m/s, starting ~12 deg from stall. Ice
# removed ~5 deg and left ~7, never reaching the 5.0 deg caution threshold.
#
# The cause was scope, not physics. Cruise is the highest-margin phase of
# flight, and the icing accident record does not sit there: Cao Table 15's
# encounters and NTSB AAR-96/01 (Roselawn, a HOLDING pattern) are slow-flight
# events where margin is small before ice contributes anything.
#
# The flight profile is NOT a use case. A use case is the decision being asked
# of the system ("is departure advisable?", "act now?"); the profile is where in
# the flight envelope that decision is taken. They are independent, so the
# profile is sampled separately and recorded as `scn_flight_profile`.
#
# Speeds are multiples of the DHC-6's published clean stall range (40-47 m/s,
# airframes.py) at standard operational factors: 1.30 Vs approach, 1.35 Vs hold,
# 1.40 Vs climb. They are not tuned to produce warnings, and the envelope
# thresholds in aircraft.py are untouched (5.0 deg caution, 2.5 deg warning).

FlightProfile = Literal["cruise", "approach", "hold", "climb"]

FLIGHT_PROFILES: tuple[str, ...] = ("cruise", "approach", "hold", "climb")


def _make_flight_plan(
    profile: str,
    rng: np.random.Generator,
    *,
    base_alt_m: float,
    duration_s: float,
    v_stall_ms: float,
) -> tuple[FlightPlan, float]:
    """Build the guidance schedule for one flight profile.

    Returns the plan and the altitude the icing layer should be centred near,
    so the scenario builder can place its cell where the aircraft actually
    flies rather than where it started.
    """
    ...
    if profile == "cruise":
        return (
            FlightPlan(
                segments=[(0.0, base_alt_m, 77.0, 0.0, False, "cruise")],
                heading_rad=heading,
            ),
            base_alt_m,
        )
    ...
    if profile == "approach":
        # Descent to circling minima. Flaps and gear each consume margin
        # before ice contributes anything.
        v_app = 1.30 * v_stall_ms
        top = max(base_alt_m, 2400.0)
        return (
            FlightPlan(
                segments=[
                    (0.0, top, 1.45 * v_stall_ms, 0.0, False, "cruise"),
                    (0.45 * duration_s, 1500.0, 1.35 * v_stall_ms, 10.0, False, "descent"),
                    (0.72 * duration_s, 700.0, v_app, 20.0, True, "approach"),
                    (0.88 * duration_s, 400.0, v_app, 20.0, True, "approach"),
                ],
                heading_rad=heading,
            ),
            float(rng.uniform(900.0, min(top, 2600.0))),
        )

    if profile == "hold":
        # Racetrack hold in icing -- the Roselawn configuration.
        hold_alt = float(np.clip(base_alt_m, 1800.0, 3000.0))
        return (
            FlightPlan(
                segments=[(0.0, hold_alt, 1.35 * v_stall_ms, 0.0, False, "cruise")],
                heading_rad=heading,
            ),
            hold_alt + float(rng.uniform(-300.0, 300.0)),
        )

    if profile == "climb":
        # Climb at max continuous through the layer. There is no spare thrust,
        # so added drag cannot be answered with power: the aircraft falls behind
        # its commanded profile and decays toward stall.
        v_climb = 1.40 * v_stall_ms
        top = max(base_alt_m, 3500.0)
        return (
            FlightPlan(
                segments=[
                    (0.0, 800.0, v_climb, 10.0, False, "climb"),
                    (0.15 * duration_s, top, v_climb, 0.0, False, "climb"),
                ],
                heading_rad=heading,
            ),
            float(rng.uniform(1500.0, min(top, 3800.0))),
        )

    raise ValueError(f"unknown flight profile: {profile!r}")
...
def build_uc1(
    ...
    *,
    ...
    # DHC-6 clean 1g stall range, airframes.py / RLEA_PROJECT_CONTEXT Sec. 3.
    v_stall = float(rng.uniform(40.0, 47.0))
    if flight_profile == "cruise":
        # UC-1's own departure profile: the decision is about DEPARTING into
        # this weather, so the default keeps takeoff and the initial climb.
        plan = FlightPlan(
            segments=[
        ...
        plan, cruise_alt = _make_flight_plan(
            flight_profile, rng, base_alt_m=cruise_alt,
            duration_s=duration_s, v_stall_ms=v_stall,
        )

    return ScenarioSpec(
        scenario_id=f"UC1_{regime}_{flight_profile}",
        use_case="UC1",
        description=(
...
def build_uc2(
    ...
    *,
    ...
    v_stall = float(rng.uniform(40.0, 47.0))
    plan, chosen_alt = _make_flight_plan(
        flight_profile, rng, base_alt_m=chosen_alt,
        duration_s=duration_s, v_stall_ms=v_stall,
    )

    return ScenarioSpec(
        scenario_id=f"UC2_{regime}_{'above' if prefer_above else 'below'}_{flight_profile}",
        use_case="UC2",
        description=(
...
def build_uc3(
    ...
    *,
    ...
    cell = WeatherCell(
        ...
        # 30% of episodes sample MVD in the Appendix O freezing-drizzle band,
        # so the SLD ridge mechanism in icing.py (A-ICE-17) can trigger at all.
        # 50 um is the Appendix C/O threshold, not a tuned value.
        mvd_m=(
            float(rng.uniform(55.0, 140.0)) if rng.random() < 0.30
    ...
    return ScenarioSpec(
        scenario_id=f"UC3_{regime}_{flight_profile}",
        use_case="UC3",
        description=(
